"""
OpenAI Client для інтеграції з GPT-4
"""
...
class OpenAIClient:
    ...
    def _setup_client(self):
        ...
        try:
            ...
            if not api_key:
                logger.warning("OpenAI API ключ не налаштований")
                return
            
            self.client = openai.OpenAI(api_key=api_key)
            logger.info("OpenAI клієнт успішно налаштований")
            
        except Exception as e:
            logger.error(f"Помилка налаштування OpenAI клієнта: {e}")
            self.client = None
    
    async def generate_proposal(self, job_data: Dict[str, Any], user_profile: Dict[str, Any], 
                              template: Optional[str] = None) -> Dict[str, Any]:
        """Генерація пропозиції за допомогою GPT-4"""
        try:
            if not self.client:
                raise Exception("OpenAI клієнт не налаштований")
    ...
    async def analyze_job(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        """Аналіз вакансії за допомогою GPT-4"""
        try:
            if not self.client:
                raise Exception("OpenAI клієнт не налаштований")
    ...
    async def filter_jobs(self, jobs: List[Dict[str, Any]], user_profile: Dict[str, Any], 
        ...
        """Фільтрація вакансій за допомогою GPT-4"""
        try:
            if not self.client:
                raise Exception("OpenAI клієнт не налаштований")
...
"""
    ...
    async def _call_gpt4(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "Ти - експерт з Upwork та фрілансингу. Надавай корисні та практичні поради."},
            ...
            logger.error(f"Помилка виклику GPT-4: {e}")
            raise
    ...
    def _parse_proposal_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Не вдалося розпарсити JSON відповідь")
            return {
                "proposal_text": response,
                "estimated_hours": "Не вказано",
                "proposed_rate": "Не вказано",
    ...
    def _parse_analysis_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Не вдалося розпарсити JSON відповідь аналізу")
            return {
                "complexity_score": 5,
                "budget_adequacy": "unknown",
                "requirements_clarity": "unclear",
    ...
    def _parse_filtering_response(self, response: str, original_jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Не вдалося розпарсити JSON відповідь фільтрації")
            return original_jobs[:5]
    
    def is_available(self) -> bool:
        """Перевірка доступності OpenAI API"""
